...
export class MusicStore {
  ...
  private storeScores(base: string, suffix: 'plain' | 'foot') {
    ...
    dir.forEach((d) => {
      if(d.isFile() && Path.extname(d.name).toUpperCase() == '.JSON') {
        ...
        let laneWatchers =
          score.notes.map(lane => new LaneWatcher(lane)) as [LaneWatcher, LaneWatcher, LaneWatcher, LaneWatcher];

	let maxNormalScore =
	  score.notes.map(lane => lane.length * point.perfect).reduce((acc, val) => acc + val);

        let slideCheckpoints = this.loadSlideCheckpoints(score.slideNotes);

	let maxSlideScore =
	  slideCheckpoints.map(checkpoints =>
	    checkpoints.reduce((acc, cp) => acc + cp.point, 0)
	  ).reduce((acc, val) => acc + val);

	let slideWatchers =
	  slideCheckpoints.map(checkpoints => new SlideWatcher(checkpoints)) as [SlideWatcher, SlideWatcher, SlideWatcher, SlideWatcher];
  ...
  private loadSlideCheckpoints(slideNotes: SlideNote[]): [SlideCheckpoint[], SlideCheckpoint[], SlideCheckpoint[], SlideCheckpoint[]] {
    let result = [[], [], [], []] as [SlideCheckpoint[], SlideCheckpoint[], SlideCheckpoint[], SlideCheckpoint[]];
  ...
  private splitSlideNote(note: SlideNote): SlideCheckpoint[] {
	...
	noteId: note.id,
        expires: t,
	lane: note.startLane + (note.endLane - note.startLane) * ((t - note.startExpires) / (note.endExpires - note.startExpires)),
	point: point.perfect *  (slide.delta / (note.endExpires - note.startExpires))
      });
    }
    return result;
  }
