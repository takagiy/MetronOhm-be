...
export class Play {
  ...
  constructor(music: Music, sender: Sender, onEnd: () => void) {
    ...
    this.watchers.laneWatchers.forEach(w => w.init(this.startTime, this.judgeResolver(sender)));
    setTimeout(() => sender.send('/input', { command: 'LRFUp' }), this.startTime - Date.now());

    new Promise( r => sender.send('/select-music', {
      command: 'start',
      time: this.startTime
    }, () => r())).then(() => {
  ...
  judge(lane: number, time: number, sender: Sender) {
    this.watchers.laneWatchers[lane].judge(time - this.startTime, this.judgeResolver(sender));
  }
  ...
  private calculateRank(): Rank {
    ...
    return ratio >= achievement.SSS / 100 ? 'SSS' :
	   ratio >= achievement.SS  / 100 ? 'SS'  :
	   ratio >= achievement.S   / 100 ? 'S'   :
	   ratio >= achievement.A   / 100 ? 'A'   :
	   'B';
  }
}
