...
export const otogeReceivable: ReceivableMap = {
  ...
  '/select-music': {
    selectMusic(arg: { name: string, difficulty?: Difficulty, foot: boolean }, model: OtogeModel, sender: Sender) {
      console.log(`selectMusic::receive ${arg.foot}`);
      model.nowPlaying = new Play(model.musicStore.get(arg.name, arg.foot, arg.difficulty), sender, () => {
  ...
  '/input': {
    lrf(arg: { xs: [number, number] }, model: OtogeModel, sender: Sender) {
      ...
      if(model.state == 'playing' && model.nowPlaying) {
        sender.send('/foot', { command: 'foot', positionLeft: arg.xs[0] - 2, positionRight: arg.xs[1] - 2});
